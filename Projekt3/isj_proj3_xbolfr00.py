#!/usr/bin/env python3



# ukol za 2 body
def first_odd_or_even(numbers):
    """Returns 0 if there is the same number of even numbers and odd numbers
       in the input list of ints, or there are only odd or only even numbers.
       Returns the first odd number in the input list if the list has more even
       numbers.
       Returns the first even number in the input list if the list has more odd
       numbers.

       "" "Vráti 0, ak existuje rovnaký počet párnych a nepárnych čísel
        vo vstupnom zozname ints alebo sú iba nepárne alebo iba párne čísla.

        Vráti prvé nepárne číslo vo vstupnom zozname, ak má zoznam viac párnych čísel
        Čísla.

        Vráti prvé párne číslo zo zoznamu vstupov, ak je zoznam nepárny
        Čísla.

    >>> first_odd_or_even([2,4,2,3,6])
    3
    >>> first_odd_or_even([3,5,4])
    4
    >>> first_odd_or_even([2,4,3,5])
    0
    >>> first_odd_or_even([2,4])
    0
    >>> first_odd_or_even([3])
    0
    """
    neparne = 0
    parne = 0
    for cisla in numbers:
        if cisla % 2 == 0:
            parne = parne + 1
        else:
            neparne = neparne + 1
    if parne == neparne or neparne == 0 or parne == 0:
        return 0
    if parne > neparne:
        for i in numbers:
            if i % 2 != 0:
                return i
    else:
        for x in numbers:
            if x % 2 == 0:
                return x

# ukol za 3 body
def to_pilot_alpha(word):
    """Returns a list of pilot alpha codes corresponding to the input word

    >>> to_pilot_alpha('Smrz')
    ['Sierra', 'Mike', 'Romeo', 'Zulu']
    """

    pilot_alpha = ['Alfa', 'Bravo', 'Charlie', 'Delta', 'Echo', 'Foxtrot',
        'Golf', 'Hotel', 'India', 'Juliett', 'Kilo', 'Lima', 'Mike',
        'November', 'Oscar', 'Papa', 'Quebec', 'Romeo', 'Sierra', 'Tango',
        'Uniform', 'Victor', 'Whiskey', 'Xray', 'Yankee', 'Zulu']

    pilot_alpha_list = []
    for letter in word:
        for slovo in pilot_alpha:
            if letter.upper() == slovo[0]:
                pilot_alpha_list.append(slovo)
    return pilot_alpha_list


if __name__ == "__main__":
    import doctest
    doctest.testmod()
